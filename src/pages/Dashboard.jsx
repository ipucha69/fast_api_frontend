...
const styles = `
  ...
  * {
  ...
  .logout-btn:hover {
    ...
    transform: translateY(-1px);
    box-shadow: 0 0 20px rgba(255, 80, 100, 0.15);
  }
  ...
  /* ── TABLE SECTION ── */
  .table-section { padding: 0 48px 64px; }
  ...
  .row-index { text-align: center; color: #303850; font-size: 12px; }
  ...
  .name-text { color: #fff; font-weight: 500; }
  ...
  .empty-desc { font-size: 13px; color: #303850; }

  /* ── LOADING ── */
  .loading-dots { display: flex; gap: 6px; justify-content: center; padding: 48px; }
...
export default function Dashboard() {
  ...
  const fetchUsers = () => {
    setLoading(true);
    API.get("/users")
      .then((res) => { setUsers(res.data); setFetched(true); })
      .catch(() => alert("Fetching workmates failed"))
      .finally(() => setLoading(false));
  };
    ...
    <>
      ...
      <div className="dashboard">
        ...
        <section className="dashboard-hero">
          <div className="hero-glow" />
          <div className="hero-label">Team Directory</div>
          <h1 className="hero-title">Your <span>Workmates</span></h1>
          <p className="hero-subtitle">Browse and connect with everyone on your team.</p>
        </section>
                ...
                <tbody>
                  {users.map((u, i) => (
                    <tr key={u.id ?? u.email} style={{ animationDelay: `${i * 40}ms` }}>
                      ...
                      <td>
                        <div className="name-cell">
                          <div className="row-avatar" style={{ background: avatarGradients[i % avatarGradients.length] }}>
                            {getInitials(u.name, u.email)}
                          </div>
                          <div className="name-text">
                            {u.name || "—"}
